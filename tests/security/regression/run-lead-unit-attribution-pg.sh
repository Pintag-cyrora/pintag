...
# 20260921000000_lead_unit_attribution.sql
#
...
# schema_lead_unit_attribution.sql (a trimmed properties/unit_types/
# contacts/parties/lead_events/leads fixture matching today's pre-migration
# shape), applies the migration TWICE in a row (proving it is idempotent),
# then runs the assertions in lead_unit_attribution_regression.sql. The
...
ROOT="$(cd "$HERE/../../.." && pwd)"
MIGRATION="$ROOT/supabase/migrations/20260921000000_lead_unit_attribution.sql"
...
echo "Loading fixture (trimmed pre-migration lead_events/leads/unit_types schema)…"
"${PSQL[@]}" -q -f "$HERE/schema_lead_unit_attribution.sql"
...
if "${PSQL[@]}" -f "$HERE/lead_unit_attribution_regression.sql" 2>&1 | sed 's/^psql:.*NOTICE: *//'; then
  echo
  echo "PASS — unit_type_id/unit_id thread correctly through lead_events -> leads, migration idempotent, existing recipient attribution unaffected."
else
  echo
