...
- **Business-first architecture.** Marketing OS exists to run our own businesses first, not as a SaaS product — see `ARCHITECTURE.md` §0 and `brain/ceo.md`'s Non-Negotiable Principles. The test is: *does a business we actually run need this today?* Defer things only a future **customer** would need — multi-tenancy, an external API surface, team roles, billing (§12). **Do not defer things the founder needs to operate the business today.** Specifically core, not adapters: operating Marketing OS from a phone (it is the primary interface — the Mac builds Marketing OS, it is not where Marketing OS runs), departments executing continuously without the founder present (the Morning Brief is an executive briefing on *already-completed* work, which requires it), and Marketing OS holding its own production credentials. §0's v2.6 correction has the full reasoning — earlier revisions mis-drew this line as "local = core, cloud = adapter," which contradicted the product.
