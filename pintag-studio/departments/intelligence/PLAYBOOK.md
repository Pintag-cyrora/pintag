...
A second real consumer as of the Daily Briefing (`pipeline/daily-briefing.ts`, CMO agent): read-only, calls `loadAllKnowledgeEntries()` (what was newly verified) and `listPendingSuggestions()` (what's waiting on review) to compose the founder's daily narrative. Intelligence doesn't know or care that this caller exists beyond that — same agent-agnostic design as everything else in this department.
