...
1. **Continuous Knowledge Capture** — not yet built. Wire `proposeKnowledgeEntry()` into Writer, Brand Guardian, and Marketing Analyst. Still the highest-priority remaining item: it's what turns capture from "one stage does it" into "every workflow does it."
...
**SOP 1 — Researcher's retrieval + capture loop** (`pipeline/stages/02-research.ts`, unchanged this sprint):
