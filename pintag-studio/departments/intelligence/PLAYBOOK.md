...
**Software exists as of this sprint; real daily use hasn't started yet — that's the honest distinction this section has to hold.** Knowledge capture happens as a side effect of `daily-content-pipeline.yml` running Stage 02. The intended daily routine, now actually possible: morning — run `npm run knowledge:review` to work through anything captured overnight (approve/reject/merge); run `npm run knowledge:dashboard` to refresh the control-center snapshot. No dedicated evening routine planned. This routine has not yet been exercised on a real calendar cadence — see §19 and `../_GRADUATION_CHECKLIST.md`'s "two consecutive weeks" criterion, which is what actually promotes this section from "possible" to "true."
...
**Current level: 2 — Software Exists.** As of this sprint, every piece of software the Level 3 push required now exists and its mechanism has been verified end-to-end (`knowledge.ts`, the `brain/lao` adapter, Stage 02 wiring, plus the new Knowledge Review Queue and the Dashboard/Explorer). That makes the department **ready to operate**, not **operated** — per `../_GRADUATION_CHECKLIST.md`, maturity is measured by operation, not implementation, and the two-consecutive-weeks daily-use criterion hasn't started yet. **Level stays at 2 until real usage, not this sprint's build work, earns Level 3.**
