...
A second real consumer as of the Daily Briefing (`pipeline/daily-briefing.ts`, CMO agent): read-only, calls `loadAllKnowledgeEntries()` (what was newly verified) and `listPendingSuggestions()` (what's waiting on review) to compose the founder's daily narrative — and, since the Executive Briefing Screen (`dashboard/morning.html`) is rendered from the same generation pass, the same two calls also drive that screen's "Yesterday's Win" callout and its Knowledge Suggestions preview. Intelligence doesn't know or care that either caller exists beyond that — same agent-agnostic design as everything else in this department.
