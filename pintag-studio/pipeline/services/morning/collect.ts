...
export function buildPrompt(sections: {
  ...
  return [
    ...
    '5. A short closing narrative: state the observed facts first, then — only if the evidence actually supports it — note what that might mean going forward, making clear it\'s your assessment rather than a proven trend (one or two quiet days is not itself a pattern). End with a specific next step sized to what you actually know. Never state a general content strategy (e.g. "we should be publishing X most days") as settled fact unless it\'s directly grounded in the performance data above or an explicit rule in the reference material — otherwise frame it plainly as your recommendation, not a conclusion.',
    '',
    'Keep it under 200 words. No headers/bullet-heavy dashboard formatting inside the prose — write it as something a person would actually say out loud. Report facts before interpreting them, and keep the strength of any recommendation proportional to the evidence behind it — don\'t imply causation or a trend from inactivity alone.',
    '',
