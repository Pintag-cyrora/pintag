...
- Write the Daily Briefing (`pipeline/daily-briefing.ts`, `npm run daily-briefing`) — a short, first-person, proactive report to the founder in the voice of a trusted junior strategist, not a dashboard: what's happening (framed as business implications, never naming internal system layers — see `FOUNDING_PRINCIPLES.md`'s Customer Philosophy), what's in flight, what needs attention (pending approvals, pending Knowledge Suggestions, department health). This is the real implementation of what used to be only a one-line dashboard recommendation.
- Per "Observe → Think → Recommend → Execute" (`FOUNDING_PRINCIPLES.md`), every Daily Briefing ends with exactly one structured, machine-parseable `RECOMMENDED ACTION:` line — a short imperative, written the way a future one-click button would read (e.g. "Generate Today's Educational Post," "Restore the Supabase Connection"). Never zero, never a list — the single most important thing to do if only one thing gets done.
- Stay evidence-driven: report what actually happened before interpreting it, and keep observation, assessment, and recommendation distinguishable — never imply a confirmed trend from a day or two of inactivity, and never state a general content strategy as settled fact unless it's grounded in real performance data or an explicit operating principle (`brain/ceo.md`, `brain/org-config.json`). Otherwise, frame it plainly as a recommendation, not a conclusion — and size its confidence to the evidence actually available.
- The same generation pass also renders the Executive Briefing Screen (`dashboard/morning.html`) — the smallest working version of the Executive Morning Workflow (Good Morning → Daily Briefing → Review Knowledge Suggestions → Today's Priorities → Recommended Action → Start My Day). Zero additional LLM calls: it's the same briefing text, plus a derived (not model-generated) "Today's Priorities" list (can legitimately be empty), an occasional "Yesterday's Win" callout, and the Recommended Action rendered as its own prominent, honestly-not-yet-clickable card — the seed a future real one-click execution feature attaches to, not a fake working button.
