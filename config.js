...
(function () {
  ...
  var CONFIGS = {
    production: {
      ...
      // storageOrigin / storageKey: see the window.PINTAG assignment below and
      // docs/API_PROXY.md. `url` is the API DELIVERY host and is the only one
      // of the three that moves to https://api.pintag.io at cutover.
      storageOrigin: 'https://eoladhcljbpbhnrmmpev.supabase.co',
      storageKey: 'sb-eoladhcljbpbhnrmmpev-auth-token',
    ...
    development: {
      ...
      // Dev keeps the direct origin for all three: the api.pintag.io proxy
      // fronts the PRODUCTION project only, so nothing here moves at cutover.
      storageOrigin: 'https://ebtgoqrywdywuqrvudcp.supabase.co',
      storageKey: 'sb-ebtgoqrywdywuqrvudcp-auth-token',
  ...
  window.PINTAG = {
    ...
    // STORED/PUBLIC STORAGE origin — deliberately NOT the same knob as
    // supabaseUrl. A public property-image URL is PERSISTED in
    // properties.images, so its host is part of a stored data format, not a
    // delivery choice, and it must stay pinned to the direct Supabase origin
    // whatever supabaseUrl becomes. Letting the delivery host leak into stored
    // values would split the table into two URL shapes and silently disable the
    // image CDN for every photo taken before the api.pintag.io cutover.
    storagePublicOrigin: active.storageOrigin,
    ...
    // `sb-<first DNS label of supabaseUrl>-auth-token`, so moving the API host
    // would silently change the key and sign every administrator and agent out
    // mid-session. Every createClient() call passes this explicitly; see
    // supabase-origins.test.js.
    authStorageKey: active.storageKey,
