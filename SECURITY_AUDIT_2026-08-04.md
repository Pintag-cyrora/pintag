...
| F4 | High | Storage writes gated on `admin@pintag.io` — breaks all uploads once the account is removed; not the single boundary | ⚠️ **Production verification (2026-08-04) revealed the fix never applied** — `20260804140000`'s DROPs named non-existent policies (no-ops), leaving storage **OPEN**: `agent_photos_insert` role `{public}`/`WITH CHECK true` (anon upload), plus `authenticated`-any write/delete on `property-images`/`agent-photos`. Hard-fixed by drift-proof `20260804150000` (drop-ALL-then-recreate `is_pintag_admin`). **Requires production apply + re-verify (P3).** |
